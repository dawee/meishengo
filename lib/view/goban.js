...
GobanView.prototype.initialize = function (opts) {
  ...
  this.meiGoban.on('intersection:mousedown', _.bind(function (data) {
    this.trigger('intersection:click', data);
  }, this));
  this.meiGoban.on('intersection:mousemove', _.bind(function (data) {
...
GobanView.prototype.resize = function (geom) {
  if (!!geom) this.$el.css(geom);
  
  this.meiGoban.maximize();
};
...
GobanView.prototype.render = function () {
  ...
  this.currentStones.each(function (stone) {
    this.meiGoban.removeStone(stone.get('row'), stone.get('col'), {force: false});
  }, this);

  this.currentStones = newStones;

  this.meiGoban.draw(true);
  return this;
};
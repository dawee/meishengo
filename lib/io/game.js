...
game.greetings = function (socket, data) {
  ...
  GameCache.fetch(this.gameId, _.bind(function (err, model) {
    ...
    socket.emit('game:chat:welcome', {content: [
      'Hi ' + data.nickname,
      'You just entered this room, welcome !',
      'To invite people to join this room, for playing with you or just watch the game, ' +
      'share this link: http://' + conf.get('host') + '/game/' + this.gameId
    ]});

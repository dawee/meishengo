...
game.joinGame = function (color) {
  ...
  oboe({method: 'POST', url: '/game/' + this.id + '/player', body: {color: color}}).done(function (data) {
    ...
    socket.on('game:passed:' + opponent, that.seats[color].nextPassEndGame);

    that.seats[color].on('action:pass', that.pass);
    that.seats[color].on('action:end', that.end);
    joinPopup.hide();
  });
};
...
game.end = function () {
  ...
  this.goban.countTerritories(function (territories) {

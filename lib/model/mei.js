...
Model.prototype.cast = function (attr, val) {
  ...
  if (_.isFunction(this.schema[attr]) && !(val instanceof this.schema[attr])) {
    var current = this.get(attr);

    if (current && (current instanceof this.schema[attr])) return current.set(val);
    return new this.schema[attr](val);
  }

...
Transaction.prototype.putStone = function (stone) {
  ...
  this.monitor('attachToAll');
  this.monitor('mergeGroups');
  this.monitor('removeDeadGroups');
  return this.monitor('checkLiberties');
};

Transaction.prototype.monitor = function (name) {
  var t0 = Date.now();
  var result = Transaction.prototype[name].apply(this);

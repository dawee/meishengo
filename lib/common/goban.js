...
goban.hasGroupMarkupAt = function (row, col, markup) {
  var res = false;
...
goban.hasGroupMarkup = function (group, markup) {
  return !!group[markup];
};

goban.addGroupMarkupAt = function (row, col, markup) {
  _.each(this.groups, function (group) {
    if (!group.containsPos({row: row, col: col})) return;

    group[markup] = true;
    _.each(group.toStoneArray(), function (stone) {
      this.emit('markup:' + markup, _.extend({value: true}, stone.dump()));
    }, this);
  }, this);
};

goban.rmGroupMarkupAt = function (row, col, markup) {
  _.each(this.groups, function (group) {
    if (!group.containsPos({row: row, col: col})) return;

    group[markup] = false;
    _.each(group.toStoneArray(), function (stone) {
      this.emit('markup:' + markup, _.extend({value: false}, stone.dump()));
    }, this);
  }, this);
};
...
goban.countMarkup = function (markup) {
  var that = this;
  var counters = {black: 0, white: 0};

  _.each(this.groups, function (group) {
    if (that.hasGroupMarkup(group, markup)) {
      counters[group.color()] += group.size();
    }
  });
...
goban.countTerritories = function (done) {
  ...
  var deads = this.countMarkup('dead');
  var prisoners = {
    black: this.prisoners.black + deads.black,
    white: this.prisoners.white + deads.white,    
  };

  referee.verbose = this.verbose;
  referee.countTerritories(function (territories, counters) {
    done(territories, counters, prisoners);
  });
};
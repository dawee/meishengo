...
referee.tick = function () {
  ...
  if (this.differed.length === 0) {
    ...
    var filtered = _.filter(this.territories, function (territory) {
      if (!territory.removed && territory.intersections.length > 0) {
        if (territory.color === 'black') counters.black += territory.intersections.length;
        if (territory.color === 'white') counters.white += territory.intersections.length;
    ...
    return this.done(filtered, counters);
  }

...
game.chat = function (data) {
  var player = playerStore.all()[0];

  this.layout.unlockChat();

  this.layout.on('chat:message', _.bind(function (evt) {
    this.socket.emit('game:chat:message:request', {
      message: {
        login: player.nickname,
        content: evt.content
      }
    });
  }, this));

  this.socket.on('game:chat:message', _.bind(function (evt) {
    this.layout.content.chatbox.echoMessage(evt.message);
  }, this));

  this.socket.on('game:spectator:joined', _.bind(function (evt) {
    this.layout.content.chatbox.echoMessage({
      content: evt.nickname + ' joined the room',
      type: 'system'
    });
  }, this));

  this.socket.on('game:chat:system', _.bind(function (evt) {
    this.layout.content.chatbox.echoMessage({
      content: evt.content,
      type: 'system'
    });
